...
class LoadScannetMixin(object):
    ...
    def load_scannet(self, scan_ids, pc_type, load_inst_info):
        ...
        for scan_id in scan_ids:
            # load inst
            if load_inst_info:
                inst2name = json.load(open(os.path.join(SCAN_FAMILY_BASE, 'scan_data', 'instance_id_to_name', '%s.json'%scan_id)))
                inst_labels = [self.cat2int[i] for i in inst2name]
