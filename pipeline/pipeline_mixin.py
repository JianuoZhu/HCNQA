...
class ModelOptimizationMixin(object):
    ...
    def warmup_cosine(step, warmup_step, tot_step):
        ...
        # if step <= tot_step:
        return max(0.5 * (1 + math.cos((step - warmup_step) / (tot_step - warmup_step) * math.pi)), 1e-5)  # 1e-5
        # return 1e-5
...
class ModelEvaluationMixin(object):
    def eval_scanrefer(self, epoch):
        ...
        if self.eval_task:
            eval_results = []
        for i, data_dict in enumerate(self.test_data_loader):
            # forward
    ...
    def eval_qa(self, epoch):
        eval_dict = {
            'target_metric': [], 'og_acc': [], 'txt_acc': [], 'obj_cls_raw_acc': [], 'obj_cls_pre_acc': [], 'obj_cls_post_acc': [], 'ans1_acc': [], 'ans10_acc': [],
            'cg_sum': [], 'cg_acc': [], 'cg_prec': [], 'cg_recall': [], 'cg_f1': [], 'cg_actual_positive': [], 'cg_predicted_positive': [], 'cg_true_positive': [], 'cg_total': [],
            'fg_sum': [], 'fg_acc': [], 'fg_prec': [], 'fg_recall': [], 'fg_f1': [], 'fg_actual_positive': [], 'fg_predicted_positive': [], 'fg_true_positive': [], 'fg_total': [],
            'infer_sum': [], 'infer_acc': [], 'infer_prec': [], 'infer_recall': [], 'infer_f1': [], 'infer_actual_positive': [], 'infer_predicted_positive': [], 'infer_true_positive': [], 'infer_total': []
        }
        # save results
        if self.eval_task:
            eval_results = []
        ...
        for i, data_dict in enumerate(tqdm(self.test_data_loader, file=tqdm_out)):
            # forward
            count = data_dict['obj_fts'].shape[0]
            total_count += count
            
            #calc flop
            if not self._profiled_forward:
                self._profile_once(data_dict, tag=f'epoch{epoch}')

            # vvvvvv STEP 1: DELETE OR COMMENT OUT THIS BLOCK vvvvvv
            # flag = 0
            # for j in range(count):
            #     if(data_dict["data_idx"][j] == 'train-scene0358-56' or data_dict["data_idx"][j] == 'train-scene0369-38'):
            #         flag = 1
            # if flag == 0:
            #     continue
            # ^^^^^^ STEP 1: DELETE OR COMMENT OUT THIS BLOCK ^^^^^^
            ...
            if self.eval_task:
                for j in range(count):
                    ...
                    pred_data = {
                        ...
                        "ans10_acc": data_dict["ans1_acc"],
                        "ao_acc": data_dict['infer_acc'].tolist()
                    }
                    if(pred_data['question_id'] == "train-scene0358-56" or pred_data['question_id'] == "train-scene0369-38"):
                        pred_data['cg_mask'] = data_dict['cg_mask'].tolist()
                        pred_data['fg_mask'] = data_dict['fg_mask'].tolist()
                        pred_data['infer_mask'] = data_dict['infer_mask'].tolist()
        ...
        # save eval results
        if self.eval_task:
            print(f"saving eval json")
            with open("scanqa_result_250612.json", "w") as f:
...
class ModelLossMixin(object):
    ...
    def get_qa_loss(self, data_dict):
        total_loss, og3d_loss, txt_cls_loss, obj_cls_raw_loss, obj_cls_pre_loss, obj_cls_post_loss, answer_loss, cg_loss, fg_loss, infer_loss = self.qa_loss(
            data_dict['txt_cls_logits'], data_dict['obj_cls_post_logits'], data_dict['obj_cls_pre_logits'], data_dict['obj_cls_raw_logits'], data_dict['og3d_logits'], data_dict['answer_scores'],
            data_dict['tgt_object_label'], data_dict['tgt_object_id'], data_dict['obj_labels'], data_dict['obj_masks'], data_dict['answer_label'], 
            data_dict['cg_mask'], data_dict['cg_gt'], data_dict['cg_logits'], data_dict['fg_mask'], data_dict['fg_gt'], data_dict['fg_logits'], data_dict['infer_mask'],
            data_dict['infer_gt'], data_dict['infer_logits'])
        data_dict['total_loss'] = total_loss
        data_dict['og3d_loss'] = og3d_loss
        data_dict['txt_cls_loss'] = txt_cls_loss
        data_dict['obj_cls_raw_loss'] = obj_cls_raw_loss
        ...
        data_dict['cg_loss'] = cg_loss
        data_dict['fg_loss'] = fg_loss
        data_dict['infer_loss'] = infer_loss
        return data_dict
...
class ModelMetricMixin(object):
    ...
    def get_qa_metrics(self, data_dict):
        ...
        # ours
        cg_mask = data_dict['cg_mask']
        cg_gt = data_dict['cg_gt']
        obj_masks = data_dict['obj_masks']
        cg_mask = cg_mask[obj_masks]
        cg_gt = cg_gt[obj_masks]
        device = cg_mask.device
        cg_gt = cg_gt.to(device)
        one_tensor = torch.ones_like(cg_mask)
        zero_tensor = torch.zeros_like(cg_mask)
        t_mask = torch.where(cg_mask > 0.5, one_tensor, zero_tensor)
        accuracy = torch.mean(torch.eq(t_mask, cg_gt).float())
        true_positive = torch.sum((t_mask == 1) & (cg_gt == 1)).float()
        predicted_positive = torch.sum(t_mask == 1).float()
        precision = true_positive / (predicted_positive + 1e-10)
        actual_positive = torch.sum(cg_gt == 1).float()
        recall = true_positive / (actual_positive + 1e-10)
        f1_score = 2 * precision * recall / (precision + recall + 1e-10)
        composite_score = accuracy + precision + recall + f1_score
        data_dict['cg_acc'] = accuracy
        data_dict['cg_prec'] = precision
        data_dict['cg_recall'] = recall
        data_dict['cg_f1'] = f1_score
        data_dict['cg_composite'] = composite_score
        data_dict['cg_actual_positive'] = actual_positive
        data_dict['cg_predicted_positive'] = predicted_positive
        data_dict['cg_true_positive'] = true_positive
        data_dict['cg_total'] = torch.sum(obj_masks == 1).float()
        data_dict['cg_sum'] = accuracy + precision + recall + f1_score
        fg_mask = data_dict['fg_mask']
        fg_gt = data_dict['fg_gt']
        obj_masks = data_dict['obj_masks']
        fg_mask = fg_mask[obj_masks]
        fg_gt = fg_gt[obj_masks]
        device = fg_mask.device
        fg_gt = fg_gt.to(device)
        one_tensor = torch.ones_like(fg_mask)
        zero_tensor = torch.zeros_like(fg_mask)
        t_mask = torch.where(fg_mask > 0.5, one_tensor, zero_tensor)
        accuracy = torch.mean(torch.eq(t_mask, fg_gt).float())
        true_positive = torch.sum((t_mask == 1) & (fg_gt == 1)).float()
        predicted_positive = torch.sum(t_mask == 1).float()
        precision = true_positive / (predicted_positive + 1e-10)
        actual_positive = torch.sum(fg_gt == 1).float()
        recall = true_positive / (actual_positive + 1e-10)
        f1_score = 2 * precision * recall / (precision + recall + 1e-10)
        composite_score = accuracy + precision + recall + f1_score
        data_dict['fg_acc'] = accuracy
        data_dict['fg_prec'] = precision
        data_dict['fg_recall'] = recall
        data_dict['fg_f1'] = f1_score
        data_dict['fg_composite'] = composite_score
        data_dict['fg_actual_positive'] = actual_positive
        data_dict['fg_predicted_positive'] = predicted_positive
        data_dict['fg_true_positive'] = true_positive
        data_dict['fg_total'] = torch.sum(obj_masks == 1).float()
        data_dict['fg_sum'] = accuracy + precision + recall + f1_score
        infer_mask = data_dict['infer_mask']
        infer_gt = data_dict['infer_gt']
        obj_masks = data_dict['obj_masks']
        infer_mask = infer_mask[obj_masks]
        infer_gt = infer_gt[obj_masks]
        device = infer_mask.device
        infer_gt = infer_gt.to(device)
        one_tensor = torch.ones_like(infer_mask)
        zero_tensor = torch.zeros_like(infer_mask)
        t_mask = torch.where(infer_mask > 0.5, one_tensor, zero_tensor)
        accuracy = torch.mean(torch.eq(t_mask, infer_gt).float())
        true_positive = torch.sum((t_mask == 1) & (infer_gt == 1)).float()
        predicted_positive = torch.sum(t_mask == 1).float()
        precision = true_positive / (predicted_positive + 1e-10)
        actual_positive = torch.sum(infer_gt == 1).float()
        recall = true_positive / (actual_positive + 1e-10)
        f1_score = 2 * precision * recall / (precision + recall + 1e-10)
        composite_score = accuracy + precision + recall + f1_score
        data_dict['infer_acc'] = accuracy
        data_dict['infer_prec'] = precision
        data_dict['infer_recall'] = recall
        data_dict['infer_f1'] = f1_score
        data_dict['infer_composite'] = composite_score
        data_dict['infer_actual_positive'] = actual_positive
        data_dict['infer_predicted_positive'] = predicted_positive
        data_dict['infer_true_positive'] = true_positive
        data_dict['infer_total'] = torch.sum(obj_masks == 1).float()
        data_dict['infer_sum'] = accuracy + precision + recall + f1_score
